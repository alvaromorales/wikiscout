...
class START:
    ...
    def format_request(self,query,action,kb='yes',machine='nauru.csail.mit.edu',server='guest'):
        request = '(:REMOTE-ADDRESS "%s" :VB "normal" :QUERY %s :USER_AGENT "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_8_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.69 Safari/537.36" :TE "XML" :KB "%s" :REFERRER "http://nauru.csail.mit.edu/wikiscout" :DE "no" :ACTION "%s" :FG "yes" :SERVER "%s" :QE "HTML" :EQ "history" :QI "qsymbols" :MACHINE "%s" :HTTP_HOST "groups.csail.mit.edu")'%(self.remote_address,json.dumps(query),kb,action,server,machine)
        return request
