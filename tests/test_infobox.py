...
class TestNormalizeTemplate(unittest.TestCase):
    def test_caps(self):
        self.assertEqual(infobox.normalize_template('INFOBOX_template'),'infobox_template')

    def test_underscores(self):
        self.assertEqual(infobox.normalize_template('infobox__officeholder'),'infobox_officeholder')

    def test_spaces(self):
        self.assertEqual(infobox.normalize_template(' INFOBOX__PERSON '),'infobox_person')

class TestNormalizeAttribute(unittest.TestCase):
    def test_valid(self):
        self.assertEqual(infobox.normalize_attribute('birth_date'),'birth_date')
...
class TestValidationRules(unittest.TestCase):
    ...
    def test_valid_template(self):
        self.assertTrue(infobox.validate_template('infobox_officeholder'))

    def test_invalid_template(self):
        self.assertFalse(infobox.validate_template('infobox_person/template'))
...
class TextInfoboxParsing(unittest.TestCase):
    ...
    def test_get_attributes_raw(self):
        article_infobox = infobox.parse(self.article['infobox']['name'], self.article['infobox']['description'])
        self.assertItemsEqual(infobox.get_attributes(article_infobox,raw=True),[('name','NAME'),('vicepresident','vicepresident'),('term_start','term___start'),('no_value','no_value')])
    ...
    def test_get_all_attributes_raw(self):
        article_infobox = infobox.parse(self.article['infobox']['name'], self.article['infobox']['description'])
        self.assertItemsEqual(infobox.get_all_attributes(article_infobox,raw=True),[('name','NAME'),('vicepresident','vicepresident'),('term_start','term___start'),('no_value','no_value'),('1','1')])
